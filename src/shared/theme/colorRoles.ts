 ...
 *
 * Este archivo es la ÚNICA fuente de verdad de los valores hex reales (nativo
 * y la hoja de estilos de `+html.tsx` en web parten de acá). Vive con un
 * nombre neutro (no `colors.ts`) a propósito: `colors.ts`/`colors.web.ts` sí
 * son el par que Metro resuelve por plataforma, y `+html.tsx` necesita los
 * valores hex reales incluso cuando compila para web (para generar las
 * variables CSS), así que no puede depender de ese par sin arriesgar que se
 * resuelva a sí mismo.
 */
...
/** Nombre de la variable CSS custom property para un rol dado. */
